...
- **Individual product pages are generated (v0.6.0) — never hand-edit a file under `products/<category>/*.html`.** Edit `scripts/products-data.js` (content) or `scripts/product-template.js` (layout, shared by all 24 pages), then run `node scripts/generate-products.js` from the repo root. Every generated file has a header comment saying so; if you're about to edit inside `products/utility-sheds/` (etc.) directly, stop and edit the data/template instead.
- `scripts/` is dev-time-only Node tooling (stdlib only, zero npm dependencies) — it is not part of the served site and never runs at deploy time. Don't add a `package.json` with dependencies for this; keep the generator dependency-free.
- Product pages live two folders deep (`products/<category>/<slug>.html`): root asset/page links need `../../`; the category page needs `../<category>.html`; sibling products in the same category need no prefix; products in a *different* category need `../<other-category>/<slug>.html`. `scripts/product-template.js` already gets this right — copy its patterns rather than re-deriving paths by hand.
- Category pages and product pages both use a `<nav class="breadcrumb container">` (see `css/components/product-page.css`) — keep it in sync with the actual folder depth when adding a new page under `products/`.
- When a copy/content bug affects the shared template (e.g. a grammar issue with certain product names), fix it once in `scripts/product-template.js` or `scripts/products-data.js` and re-run the generator — don't patch individual generated files, they'll just be overwritten and the bug will resurface for anyone who regenerates later.
